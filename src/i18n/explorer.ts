...
const zhHant: ExplorerCopy = {
  ...
  harbourPlan: {
    ...
    note: (rooms, enumerated) =>
      `底圖是實景航照圖（縮放層級 19，1 像素 0.24 公尺，比例尺 100 公尺，是浦安一帶目前最清晰的公開影像），不是手繪的形狀，所以海港、廣場、街屋都是照片本身。房間的位置來自實測而非來源圖的角度：來源的手繪圖是示意的，同一份圖裡每間房畫出來的寬度可以差到一倍半，所以它可信的是順序與轉折在第幾間，不是長度與角度。實際的長度與角度用牆。從西北翼到西南端這一整面「地中海港灣側」外牆，OpenStreetMap 輪廓給了 31 個點、294 公尺；那些點多半是 2 到 9 公尺深的陽台與凸窗，留著會讓牆成鋸齒，一間房橫跨其中一個直角就會壓到隔壁。所以先按各段的方位角把點分組，一組配一條最小平方線，相鄰兩線相交處就是轉角，而不是對整面牆硬配固定條數的線。這一步是這次改動的關鍵：後者配出的六條線把西北翼的兩個點算進了北向走廊那條線，兩者之間的轉角因此往西偏了 6 公尺，而每個翼再按總長分到房位，西北翼就整整越過自己的轉角、沿北向走廊多長出三間房。按方位角分組之後，每個轉角都落在來源圖轉折的位置，31 個點全部落在自己那面牆的 2.5 公尺以內（原來最遠 4.2 公尺），六面牆合計 255.1 公尺。每個翼站在自己那一面牆上，房間面寬因此是這個翼自己的實測值而不是全棟平均：3.99、4.11、4.39、4.47 公尺，對照 37 平方公尺客房在 8.5 公尺深時的 4.4 公尺面寬——四面牆都差不到一成，而且沒有為了對上而調整任何數字。原本只有西北翼對不上，寬到 4.87 公尺；重新逐格判讀來源圖才知道原因：那一翼折角處靠港灣那一側有一格是純牆面，4 樓畫在 4147 與 4145 之間、5 樓畫在 5149 與 5145 之間，原本漏算了這一格。來源圖把西北翼畫成：轉折前兩側各四間房，然後轉約 20 度，接著是那一格牆，再接十一間；輪廓在同一處也轉，而那面牆再延伸出去就是托斯卡納側（那一側的房號不在這份圖裡），所以只用它最靠近轉角的四間房。這面牆上共 60 個房位，加上折角外的四間。房深畫 8.8 公尺，加 2 公尺走廊，整個翼寬 19.6 公尺。一條走廊兩側的房間共用同一組房位，因為那本來就是同一批結構開間從走廊兩邊看：每一格都逐一寫出來，來源圖上是電梯廳、樓梯間或酒廊而不是客房的，就寫成空格。這是 4306 之所以正對 4305、而不是從南向走廊的第一格開始的原因，也順帶釐清了幾筆房號：4346 與 4360 不存在，兩者都落在西南翼轉折處，飯店自己的房型清單也從來沒有這兩個號；4330 與 4332 存在但原本漏掉，它們和 4334 是內側那一排比港灣側提早轉出南向走廊的三間；4123 屬於北向走廊而不是西北翼，來源圖把它與走廊擺正，並讓 4122 正對 4121；東南翼走廊的起點加電梯廳佔掉的是前兩格而不是一格，所以 4405 正對 4406、4419 正對 4420。牆的每個轉折都移到最近的房間交界（不超過半間），兩側同時採用同一個交界，這樣沒有房間需要橫跨轉折；在轉折處相接的兩間房沿角平分線分界，就是來源圖畫的楔形房。走廊內側那一排離牆較遠，牆往它轉的時候，它可站的長度就短一截——等於離牆距離乘以轉角一半的正切再乘二，在港灣臂上每個轉折大約剛好是一間房——來源圖畫的正是它就地收攏、不留缺口：4344 之後直接是 4348、4358 之後直接是 4362，少掉的號碼並沒有留出空位。所以房間維持牆給的面寬，轉折那一格只拿剩下的長度：西北翼折角處 2.5 公尺，港灣臂上 2.1 公尺或完全沒有。這是走廊兩排能逐格對齊的原因，也是這張圖到現在才改對的地方——原本在來源圖沒有缺口的位置畫出了一整間房寬的洞。兩個翼接在建物同一個轉角上時也一樣：各自對自己那面牆切直角的話，轉彎內側的兩間房會同時佔掉那 25 平方公尺，所以每個翼都知道牆往哪邊繼續，兩邊沿同一條角平分線分。若這樣還是留不下背牆——西北翼與北向走廊之間轉 43 度，而內側那一排在牆內 19 公尺處——那兩間就只畫到留得下背牆的深度，往轉角逐漸變淺。八段走廊有六段站在實測牆上，另外兩段沒有：教堂東側那段是把北向走廊的牆延伸過教堂（來源圖就是這樣畫的），東南翼那段則由南向走廊背面連到實測尾牆的起點，這兩段畫成虛線。全圖 ${rooms} 格都落在實測輪廓內，彼此不重疊。每間房的順序與朝向依來源手繪平面圖描繪、房號逐格核對，其中 ${enumerated} 間能對到官方房型。手機請左右滑動。`,
...
const ja: ExplorerCopy = {
  ...
  harbourPlan: {
    ...
    note: (rooms, enumerated) =>
      `下地は実際の航空写真です（ズーム 19、1 px が 0.24 m、スケールバー 100 m。浦安周辺で公開されている最も精細な写真です）。ハーバー・広場・街並みは描いたものではなく写真そのものです。客室の位置は出典図の角度ではなく実測によります。出典の手描き図は模式的で、同じ図の中でも 1 室の描かれた幅が 1.5 倍違います。信頼できるのは順序と「何室目で曲がるか」で、長さと角度ではありません。長さと角度には壁を使いました。北西の翼から南西端まで、ポルト・パラディーゾ側の外壁は OpenStreetMap の輪郭で 31 点・294 m。その多くが奥行き 2〜9 m のバルコニーや出窓で、残すと壁が鋸歯状になり、その直角をまたいで描いた客室は隣室に重なります。そこで、まず各区間の方位角で点をまとめ、まとまりごとに最小二乗の直線を当て、隣り合う直線の交点を角としました。壁面全体に決めた本数の直線を当てるのではありません。この違いが今回の要点です。後者で当てた 6 本は北西の翼の 2 点を北側廊下の直線に含めてしまい、両者の角が 6 m 西へずれ、さらに各翼が全長の按分で客室位置を受け取るため、北西の翼は自分の角を越えて北側廊下沿いに 3 室ぶん伸びていました。方位角でまとめると角はいずれも出典図が曲がる位置に落ち、31 点すべてが自分の壁から 2.5 m 以内（以前は最大 4.2 m）、6 面で計 255.1 m です。各翼は自分の壁に載るので、1 室の間口は建物全体の平均ではなくその翼自身の実測値になります。3.99・4.11・4.39・4.47 m で、奥行き 8.5 m とした 37 m² の客室の間口 4.4 m に対し、4 面すべてが 1 割も違いません（合わせるための調整は一切していません）。合わなかったのは北西の翼だけで、4.87 m ありました。出典図をもう一度 1 マスずつ読んで理由が分かりました。あの翼の折れ曲がりのハーバー側に壁だけの 1 マスがあり、4 階では 4147 と 4145 の間、5 階では 5149 と 5145 の間に描かれています。それを数え落としていたのです。出典図は北西の翼を、曲がる前が両側 4 室、そこで約 20 度折れ、その壁だけの 1 マス、さらに 11 室と描きます。輪郭も同じ位置で折れ、その壁をさらに延ばすとトスカーナ・サイド（そちらの部屋番号はこの図に含まれません）なので、角に最も近い 4 室ぶんだけを使っています。この壁面には 60 の客室位置があり、折れ曲がりの外側にさらに 4 室です。奥行きは 8.8 m、廊下 2 m を加えて翼の幅は 19.6 m です。廊下の両側の客室列は、同じ 1 組の客室位置に対して読みます。もともと同じ構造スパンを廊下の両側から見たものだからです。各位置を 1 マスずつ書き出し、出典図がエレベーターホール・階段室・ラウンジとしているところは空白にしています。4306 が南側廊下の先頭からではなく 4305 の正面に来るのはこのためで、あわせていくつかの部屋番号も確定しました。4346 と 4360 は客室ではありません。いずれも南西の翼が曲がる位置にあたり、ホテル自身のタイプ別一覧にもこの 2 番号はありません。4330 と 4332 は客室で、これまで漏れていました。4334 とあわせて、内側の列がハーバー側より先に南側廊下の角を回る 3 室です。4123 は北西の翼ではなく北側廊下に属し、出典図は廊下に正対させて 4122 を 4121 の向かいに置いています。南東の翼では、廊下の起点とエレベーターホールが 1 マスではなく最初の 2 マスを占めます。そのため 4405 は 4406 の、4419 は 4420 の正面になります。壁の曲がり角はいずれも最寄りの客室境界（半室以内）へ寄せ、両側の列に同じ境界を与えているので、客室が角をまたぐことはありません。角で接する 2 室は角の二等分線で分けます（出典図が角に描く楔形の客室と同じです）。壁から離れた内側の列は、壁がそちらへ曲がるぶん立てる長さが短くなります。壁からの距離に曲がり角の半分のタンジェントを掛けて 2 倍した長さで、ハーバー側の腕では曲がり 1 か所あたりおおよそ 1 室ぶんです。出典図はその 1 室を詰めて描いており、4344 の次は 4348、4358 の次は 4362 で、抜けた番号の場所に隙間はありません。そこで客室は壁が与える間口を保ち、曲がりのマスには余った長さだけを渡しています。北西の翼の折れ曲がりで 2.5 m、ハーバー側の腕では 2.1 m あるいはゼロです。これが廊下の両側の列がマスごとに揃う理由で、この図がいままで間違えていた点でもあります。出典図に隙間のないところへ 1 室ぶんの穴を描いていました。2 つの翼が建物の同じ角で接するところも同じです。それぞれが自分の壁に直角で切ると、曲がりの内側で 2 室が同じ 25 m² を取ってしまうので、どちらの翼にも壁がどう続くかを渡し、1 本の二等分線で分けています。それでも背面が残らない場合——北西の翼と北側廊下の間は 43 度曲がり、内側の列は壁から 19 m 入った位置にあります——背面が残る奥行きまでしか描かず、角に向かって浅くなります。8 区間のうち 6 区間は実測の壁に載っています。残る 2 区間は、チャペル東側（北側廊下の壁をチャペルの向こうへ延ばしたもの。出典図の描き方どおり）と南東の翼（南側廊下の背面から実測の末端壁の起点まで）で、破線で描いています。全 ${rooms} マスが実測輪郭の内側にあり、互いに重なりません。各室の順序と向きは出典の手描き配置図をなぞり、部屋番号は 1 マスずつ照合、うち ${enumerated} 室が公式の客室タイプに対応します。スマートフォンでは横にスクロールしてください。`,
