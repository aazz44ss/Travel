...
const zhHant: ExplorerCopy = {
  ...
  harbourPlan: {
    ...
    note: (rooms, enumerated) =>
      `底圖是實景航照圖（縮放層級 19，1 像素 0.24 公尺，比例尺 100 公尺，是浦安一帶目前最清晰的公開影像），不是手繪的形狀，所以海港、廣場、街屋都是照片本身。房間的位置來自實測而非來源圖的角度：來源的手繪圖是示意的，對照建物輪廓，它把西北翼畫得比實際陡了約 30 度。實際用的是牆。從西北端到西南端這一整面「地中海港灣側」外牆，OpenStreetMap 輪廓給了 31 個點、294 公尺，改寫成 6 段直牆共 255.8 公尺——那些點多半是 2 到 9 公尺深的陽台與凸窗，留著會讓牆成鋸齒；轉折點取 5 公尺顯著度，其間用最小平方法配線，31 個點全部落在 4.2 公尺內、平均 1.4 公尺。來源圖在這面牆上放了 63 個房位，每間面寬算出來是 3.92 到 4.33 公尺，對照 37 平方公尺客房在 8.5 公尺深時的 4.4 公尺面寬，兩者差不到一成——這個吻合本身就是相互驗證，沒有為了對上而調整任何數字。房深畫 8.8 公尺，加 2 公尺走廊，整個翼寬 19.6 公尺。一條走廊兩側的房間共用同一組房位，因為那本來就是同一批結構開間從走廊兩邊看：每一格都逐一寫出來，來源圖上是電梯廳、樓梯間或酒廊而不是客房的，就寫成空格。這是 4306 之所以正對 4305、而不是從南向走廊的第一格開始的原因，也順帶釐清了三筆房號：4346 與 4360 不存在，兩者都落在西南翼轉折處，來源圖把那個轉角讓給走廊，飯店自己的房型清單也從來沒有這兩個號；4330 與 4332 存在但原本漏掉，它們和 4334 是內側那一排比港灣側提早轉出南向走廊的三間；4123 屬於北向走廊而不是西北翼，來源圖把它與走廊擺正，並讓 4122 正對 4121。牆的每個轉折都移到最近的房間交界（不超過半間），兩側同時採用同一個交界，這樣沒有房間需要橫跨轉折；在轉折處相接的兩間房沿角平分線分界，就是來源圖畫的楔形房，並依背牆在轉角損失的長度加寬，寬度由這一段其餘房間讓出。若這樣還是留不下背牆——西北翼轉 41 度，而內側那一排在牆內 19 公尺處——那兩間就只畫到留得下背牆的深度，往轉角逐漸變淺；來源圖把這個轉折畫在比實測往前三間房的位置，是全圖兩者差最多的地方。八段走廊有六段站在實測牆上，另外兩段沒有：教堂東側那段是把北向走廊的牆延伸過教堂（來源圖就是這樣畫的），東南翼那段則由南向走廊背面連到實測尾牆的起點，這兩段畫成虛線。全圖 ${rooms} 格都落在實測輪廓內，彼此不重疊。每間房的順序與朝向依來源手繪平面圖描繪、房號逐格核對，其中 ${enumerated} 間能對到官方房型。手機請左右滑動。`,
...
const ja: ExplorerCopy = {
  ...
  harbourPlan: {
    ...
    note: (rooms, enumerated) =>
      `下地は実際の航空写真です（ズーム 19、1 px が 0.24 m、スケールバー 100 m。浦安周辺で公開されている最も精細な写真です）。ハーバー・広場・街並みは描いたものではなく写真そのものです。客室の位置は出典図の角度ではなく実測によります。出典の手描き図は模式的で、建物輪郭と照らすと北西の翼を実際より約 30 度急に描いています。使ったのは壁です。北西端から南西端まで、ポルト・パラディーゾ側の外壁は OpenStreetMap の輪郭で 31 点・294 m ありますが、これを 6 本の直線壁・計 255.8 m に置き換えました。その 31 点の多くが奥行き 2〜9 m のバルコニーや出窓で、残すと壁が鋸歯状になるからです。曲がり角は 5 m の有意性で取り、その間の点には最小二乗で直線を当てています。31 点すべてが結果から 4.2 m 以内、平均 1.4 m です。出典図はこの壁面に 63 の客室位置を並べており、1 室の間口は 3.92〜4.33 m。奥行き 8.5 m とした 37 m² の客室の間口 4.4 m と 1 割も違わず、この一致がそのまま相互検証になります（合わせるための調整は一切していません）。奥行きは 8.8 m、廊下 2 m を加えて翼の幅は 19.6 m です。廊下の両側の客室列は、同じ 1 組の客室位置に対して読みます。もともと同じ構造スパンを廊下の両側から見たものだからです。各位置を 1 マスずつ書き出し、出典図がエレベーターホール・階段室・ラウンジとしているところは空白にしています。4306 が南側廊下の先頭からではなく 4305 の正面に来るのはこのためで、あわせて 3 つの部屋番号も確定しました。4346 と 4360 は客室ではありません。いずれも南西の翼が曲がる位置にあたり、出典図はその角を廊下に充てています。ホテル自身のタイプ別一覧にもこの 2 番号はありません。4330 と 4332 は客室で、これまで漏れていました。4334 とあわせて、内側の列がハーバー側より先に南側廊下の角を回る 3 室です。4123 は北西の翼ではなく北側廊下に属し、出典図は廊下に正対させて 4122 を 4121 の向かいに置いています。壁の曲がり角はいずれも最寄りの客室境界（半室以内）へ寄せ、両側の列に同じ境界を与えているので、客室が角をまたぐことはありません。角で接する 2 室は角の二等分線で分け（出典図が角に描く楔形の客室と同じです）、背面が角で失う分だけ間口を広げ、その分はその区間の残りの客室が譲ります。それでも背面が残らない場合——北西の翼は 41 度曲がり、内側の列は壁から 19 m 入った位置にあります——背面が残る奥行きまでしか描かず、角に向かって浅くなります。出典図はこの曲がりを実測より 3 室先に描いており、図全体で両者の差が最も大きい箇所です。8 区間のうち 6 区間は実測の壁に載っています。残る 2 区間は、チャペル東側（北側廊下の壁をチャペルの向こうへ延ばしたもの。出典図の描き方どおり）と南東の翼（南側廊下の背面から実測の末端壁の起点まで）で、破線で描いています。全 ${rooms} マスが実測輪郭の内側にあり、互いに重なりません。各室の順序と向きは出典の手描き配置図をなぞり、部屋番号は 1 マスずつ照合、うち ${enumerated} 室が公式の客室タイプに対応します。スマートフォンでは横にスクロールしてください。`,
